...
PeriodicTask::PeriodicTask(float period,std::string name,uint8_t prefer_cpu, int policy, int priority)
    : _period(period), _name(name) ,_prefer_cpu(prefer_cpu)
    {
...
void PeriodicTask::loopFunction() {
  ...
  //若输入一个大于100的值则认为不指定内核
  if(_prefer_cpu < 100)
  {
      cpu_set_t mask;
      CPU_ZERO(&mask);
      CPU_SET(_prefer_cpu, &mask);
